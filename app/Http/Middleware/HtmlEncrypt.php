...
class HtmlEncrypt
{
    ...
    public function encryptHtml($content)
    {
        ...
        if (config('html-encrypt.disable_right_click')) {
            $script .= sprintf('<script type=\'text/javascript\' nonce=\'%s\'>let body = document.getElementsByTagName(\'body\')[0];var att = document.createAttribute(\'oncontextmenu\');att.value = \'return false\'body\';body.setAttributeNode(att);</script>', $nonce);
        }
